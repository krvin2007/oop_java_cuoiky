...
**Cơ sở dữ liệu:** Được kết nối qua MySQL thông qua cấu hình tại file `controller/DBConnection.java` (URL: `jdbc:mysql://localhost:3306/quangara`).
...
*   **Minh chứng code:**
    *   `model/Person.java` (Dòng 22): Khai báo `public abstract class Person`.
    *   `model/HangMuc.java` (Dòng 20): Khai báo `public abstract class HangMuc` và định nghĩa một phương thức trừu tượng chưa có thân logic (Dòng 103): `public abstract double tinhThanhTien(int soLuong);`. Chỉ các lớp con kế thừa mới được viết logic cho phương thức này.
