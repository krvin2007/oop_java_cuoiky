...
**Mở rộng thêm:** Hệ thống còn bổ sung thêm cấu trúc phân quyền nâng cao với các thực thể mới như `Employee` (Nhân viên - Lớp cha), `Manager` (Quản lý), `Accountant` (Kế toán), `Storekeeper` (Thủ kho) để quản lý garage một cách toàn diện.
...
*   **Minh chứng code:**
    ...
    *   Bên cạnh đó, các lớp như `Manager`, `Accountant`, `Storekeeper` cũng kế thừa từ `Employee`.
...
*   **Minh chứng:** Dự án được chia thành 3 thư mục chính:
    *   **model**: 15 file (Accountant, DichVu, DuplicateMaException, Employee, HangMuc, Invoice, LinhKien, Manager, Mechanic, Owner, Person, RepairOrder, RepairOrderDetail, Storekeeper, Vehicle).
...
*   **Minh chứng:** Có rất nhiều lớp kế thừa (`Owner`, `Employee`, `Mechanic`, `Manager`, `Accountant`, `Storekeeper`, `LinhKien`, `DichVu`) và 3 lớp abstract (`Person`, `Employee`, `HangMuc`).
