...
**Thư mục gốc:** `c:\Users\CHAU TUAN KIET\Desktop\OOP_CKKK\QuanLyGara`
...
*   `view`: Chứa `GiaoDienConsole.java`, quản lý giao diện dòng lệnh, điều hướng Menu và tương tác với người dùng.

**Kết nối CSDL:** Xử lý qua file `controller/KetNoiCSDL.java`. Sử dụng JDBC kết nối tới MySQL `jdbc:mysql://localhost:3306/quangara`.

---

## 2. PHÂN TÍCH TẦNG VIEW (GIAO DIỆN)
**File:** `view/GiaoDienConsole.java` (Dài hơn 1000 dòng).
...
### 3.1. Nhóm Lớp Con Người (Nguoi - ChuXe - NhanVien)
*   **`Nguoi.java` (Lớp Trừu tượng - Abstract):**
...
*   **`NhanVien.java` và Nhóm Lớp Kế thừa (`KyThuatVien`, `QuanLy`, `KeToan`, `ThuKho`):**
    *   *Nhiệm vụ:* Quản lý nhân sự. Mở rộng thêm `username`, `password`, `salary`, `status`.
    *   `KyThuatVien.java` (Kỹ thuật viên): Bổ sung hàm nhập liệu bắt buộc `username`, `password` không được rỗng và `salary >= 0`.
...
### 3.3. Nhóm Lớp Xe và Nghiệp vụ (`Xe`, `PhieuSuaChua`, `HoaDon`)
*   **`Xe.java` (Xe Ô tô):**
...
*   **`PhieuSuaChua.java` (Phiếu sửa chữa) & `HoaDon.java` (Hóa đơn):**
...
Nằm trong package `controller/`. Các lớp DAO implement Interface `IKhoLuuTru<T>` (Sử dụng Generic). Sử dụng `PreparedStatement` để chống SQL Injection.

### 4.1. `ChuXeDAO.java` & `XeDAO.java` (Quản lý Khách hàng - Xe)
*   **Thêm mới (`themMoi`):** (Dòng ~23 `ChuXeDAO.java`) 
    *   *Nghiệp vụ chặn trùng lắp:* Trước khi thực thi `INSERT`, hàm tạo lệnh `SELECT id FROM chu_xe WHERE phone = ?`. Nếu tồn tại, lập tức `throw new Exception("Số điện thoại đã tồn tại!")`. Cấu trúc này đẩy ngoại lệ lên `GiaoDienConsole` để xử lý lùi biến đếm `i--`.
*   **Xóa mềm (Soft Delete):** (Dòng ~102)
    *   Thực hiện kiểm tra khóa ngoại (Foreign Key). Nếu khách hàng đang có xe trong xưởng (`SELECT COUNT(*) FROM xe WHERE ma_chu_xe = ?`), ném lỗi từ chối xóa. Nếu an toàn, thực hiện lệnh `UPDATE chu_xe SET da_xoa = TRUE`.

### 4.2. `PhieuSuaChuaDAO.java` & `LinhKienDAO.java` (Nghiệp vụ Cốt lõi)
*   **Tạo Phiếu (`themMoi`):** Khi tạo `PhieuSuaChua`, DAO lập tức gọi sang `KyThuatVienDAO` để đổi trạng thái của người thợ máy (KyThuatVien) sang `Đang bận` (`UPDATE nhan_vien SET status = 'Đang bận'`). Trạng thái Phiếu là `RECEIVING`.
*   **Thêm Hạng Mục Sửa Chữa (Giao dịch - Database Transaction):** 
    *   *Cơ chế cực kỳ quan trọng:* Tại hàm `addDetail` của phiếu sửa chữa, hệ thống thực thi 2 lệnh SQL: (1) `INSERT` vào bảng `chi_tiet_phieu_sua` và (2) `UPDATE` giảm trừ tồn kho trong bảng `linh_kien` (Linh kiện).
...
### 4.3. `BaoCaoController.java` (Báo cáo Thống kê)
*   *Nhiệm vụ:* Đảm nhiệm vai trò Thống kê doanh thu, TOP xe, TOP thợ.
*   *Thực thi (Ví dụ `getRevenueByPeriod`):* (Dòng ~31). Sử dụng cấu trúc lệnh SQL `SELECT SUM(tong_tien) ... FROM hoa_don WHERE ngay_thanh_toan BETWEEN ? AND ?`. Truyền hai mốc thời gian (Start Date và End Date) vào để DB tự tính toán và trả về con số.
*   *Thực thi (Ví dụ `getMostRepairedVehicles`):* (Dòng ~87). Dùng lệnh SQL `JOIN` bảng `phieu_sua_chua` với bảng `xe`, kết hợp mệnh đề `GROUP BY` biển số xe và `ORDER BY COUNT(...) DESC LIMIT 5` để lấy ra top 5 xe sửa nhiều nhất. Dữ liệu được đưa vào cấu trúc `LinkedHashMap` để giữ nguyên thứ tự sắp xếp và in ra màn hình.
...
2. **Model:** `Xe.nhapInfo()` ép lễ tân gõ đúng chuẩn biển số VN, năm sản xuất hợp lệ.
...
9. **Controller:** `HoaDonDAO.themMoi()` quét toàn bộ chi tiết, phân tách tổng tiền linh kiện (tong_tien_linh_kien) và tiền công dịch vụ (tong_tien_cong), cộng thêm 10% VAT, lưu vào bảng `hoa_don`. Trả thợ máy về trạng thái "Đang rảnh". 
...
1. **`BienSoKhongHopLeException` (Ngoại lệ Sai Định dạng Biển số):**
   - **Tác dụng:** Được kích hoạt (throw) ngay tại hàm nhập liệu `Xe.nhapInfo()` khi người dùng cố tình nhập một biển số không khớp với Regex chuẩn của Việt Nam (Ví dụ: phải có dấu gạch ngang, dấu chấm).
   - **Ý nghĩa:** Chặn dữ liệu rác ngay từ lớp Model trước khi nó kịp đẩy xuống Database.

2. **`LinhKienHetHangException` (Ngoại lệ Hết Hàng Tồn Kho):**
...
3. **`KhongTimThayXeException` (Ngoại lệ Không Tìm Thấy Xe):**
   ...
   - **Ý nghĩa:** Giúp giao diện `GiaoDienConsole` bắt chính xác lỗi thay vì nhận `NullPointerException`, từ đó in ra thông báo thân thiện "Xe không tồn tại" cho người sử dụng.
