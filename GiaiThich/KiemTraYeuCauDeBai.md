...
**Mở rộng thêm:** Hệ thống còn bổ sung thêm cấu trúc phân quyền nâng cao với các thực thể mới như `Employee` (Nhân viên - Lớp cha), `Owner` (Quản lý), `Accountant` (Kế toán), `Storekeeper` (Thủ kho) để quản lý garage một cách toàn diện.
...
*   **Minh chứng code:**
    *   Trong file `model/Person.java` (từ dòng 24): Các thuộc tính `id`, `name`, `phone`, `address` đều là `private`.
...
*   **Minh chứng code:**
    *   `model/Customer.java` (Dòng 19): `public class Customer extends Person`
    *   `model/Mechanic.java` (Dòng 5): `public class Mechanic extends Employee` (kế thừa gián tiếp Person)
    ...
    *   Bên cạnh đó, các lớp như `Owner`, `Accountant`, `Storekeeper` cũng kế thừa từ `Employee`.
...
*   **Minh chứng code:**
    ...
    *   **Overloading (Nạp chồng):** Trong `model/Customer.java` (Dòng 24 và Dòng 30) có 2 loại Constructor: `Customer()` và `Customer(int id, String name, ...)`.
...
*   **Minh chứng code:**
    *   `model/Person.java`: Khai báo `public abstract class Person` và đặc biệt là có phương thức trừu tượng `public abstract String getRoleDescription();`. Điều này ép buộc các lớp con (như `Owner`, `Employee`) bắt buộc phải ghi đè và cung cấp mô tả vai trò cụ thể của mình.
    *   `model/Employee.java`: Kế thừa `Person` và được khai báo abstract để là khuôn mẫu cho các chức vụ nhân viên khác nhau.
...
*   **Minh chứng:** Dự án được chia thành 3 thư mục chính:
    *   **model**: 15 file (Accountant, Customer, DichVu, DuplicateMaException, Employee, HangMuc, Invoice, LinhKien, Mechanic, Owner, Person, RepairOrder, RepairOrderDetail, Storekeeper, Vehicle).
    *   **controller**: 13 file (ChuXeDAO, DBConnection, DichVuDAO, EmployeeDAO, HoaDonDAO, IReportService, IRepository, KyThuatVienDAO, LinhKienDAO, PhieuSuaChuaDAO, ReportController, TestDB, XeDAO).
    *   **view**: 1 file (ConsoleView).
    *   *Tổng cộng: 29 file/lớp/interface. Vượt xa yêu cầu.*

### 3.2. Sử dụng ít nhất 2 lớp kế thừa & 1 lớp abstract
*   **Minh chứng:** Có rất nhiều lớp kế thừa (`Customer`, `Employee`, `Mechanic`, `Owner`, `Accountant`, `Storekeeper`, `LinhKien`, `DichVu`) và 3 lớp abstract (`Person`, `Employee`, `HangMuc`).
...
*   **Minh chứng code:**
    ...
    *   Tại `controller/ReportController.java` (Dòng 66, 88): Sử dụng `Map<String, Integer> result = new LinkedHashMap<>();` để lưu trữ dữ liệu dạng *Key-Value* đã được nhóm (GROUP BY).
...
*   **Minh chứng code:**
    *   Custom Exception: Tạo riêng package `exception` chứa các ngoại lệ như `DuplicateMaException.java` (ném ra khi bị trùng khóa), `PartOutOfStockException` (ném ra khi kho không đủ linh kiện), `VehicleNotFoundException`.
...
*   **Step-by-step thực thi:**
    ...
    2.  Hệ thống gọi hàm `getMostRepairedVehicles(limit)` bên trong `ReportController.java` (Dòng 87).
...
*   **Tầng DAO & Controller (Nghiệp vụ):**
    ...
    *   Chống mồ côi dữ liệu: Không cho xóa `Customer` nếu đang có xe, không cho xóa `RepairOrder` nếu đã xuất `Invoice`.
    *   Bảo vệ dữ liệu lịch sử: Chặn chỉnh sửa đối với Phiếu sửa chữa đã hoàn tất (`COMPLETED`), chặn quyền thợ (Mechanic) thao tác lên phiếu của thợ khác.
